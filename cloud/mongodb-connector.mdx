...
```bash
db.runCommand({ collMod: "orders", changeStreamPreAndPostImages: true })
```
...
You find this connection string in the MongoDB Atlas UI, under the `Connect` button for your cluster, ensuring that you use the credentials of the database user that you created in the prerequisites.
...
| `Something went wrong` | An unknown error occurred, please [contact Ditto support](/support/contact-support) for assistance. |
...
- Richer BSON datatypes are not used by the connector when documents or fields are added on the DItto side, only JSON types will be used. In the future the connector will provide the ability for users to specify the datatypes they wish to use for their documents.
