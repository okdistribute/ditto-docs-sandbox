...
description: "Access to the Ditto HTTP API is controlled through an `Authorization: Bearer HTTP` header. To authenticate and gain access to the Big Peer, use a static API key."
icon: key
iconType: solid
---
...
<Info>
  For more information about custom access rules, see [Setting Up Custom Permissions](cloud/http-api/authorization), as follows.
</Info>
  </Step>
  <Step >
...
<Info>
  Transaction IDs instruct the Big Peer on the order in which transactions should be applied. For more information, see the *Platform Manual *> [Big Peer](cloud/http-api/authorization).
</Info>
