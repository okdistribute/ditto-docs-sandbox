...
iconType: solid
---
 For an overview, see *Ditto Basics *> [CRUD Fundamentals](/v4-7/crud/create).

Use the `write` command to `upsert`, `update`, and `remove` documents from the Big Peer.

<Warning>
  Do *not *include a `find` query within a `write` command in a single operation. Instead, perform each as separate API calls. For instructions, see [Performing Find and Write Operations](/cloud/legacy/).
</Warning>
...
<Info>
For instructions on how to get your app ID, see [Get Started](/cloud/http-api/legacy/writing-http-legacy).
...
<Info>
For more information about `registers` and `counters`, see the *Platform Manual* > [Data Types (TEMP)](/cloud/http-api/legacy/writing-http-legacy).
</Info>
