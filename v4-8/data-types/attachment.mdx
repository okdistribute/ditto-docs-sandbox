---
title: ATTACHMENT
description: "This article provides conceptual information for the `ATTACHMENT` data type. For instructions on how to work with an `ATTACHMENT`, see *CRUD Operations* > [CREATE](/v4-8/crud/create), [READ](/v4-8/crud/read), [UPDATE](/v4-8/crud/update), and [DELETE](/v4-8/crud/delete)."
icon: paperclip
iconType: solid
---
...
When incorporating an attachment into a document, rather than inserting the entire resource-heavy ATTACHMENT object, internally Ditto inserts a pointer object, known as the *attachment token*. (See [ATTACHMENT Object](/v4-8/data-types/attachment#7uG0P) and [Attachment Token](/v4-8/data-types/attachment#TOjvP))

Unlike documents that are always accessible and automatically synced according to sync subscriptions, peers do not automatically fetch attachment data associated with a sync subscription. Therefore, before accessing an attachment, you must explicitly fetch it. (See [Syncing ATTACHMENT Objects](/v4-8/data-types/attachment#_Xi9V))
...
- The blob store is an array of bytes representing your large file or complex document encoded as an ATTACHMENT data type. (See [Blob Store](/v4-8/data-types/attachment#dNgRq))
...
| --- | --- | --- |
| id | Attachment ID | A cryptographic hash of the attachment's binary contents. (See [Organizing by ID](/v4-8/data-types/attachment#mehvy)) |
| len | Blob Length | The size of the blob data in terms of length (len) in bytes. (See [Blob Store](/v4-8/data-types/attachment#dNgRq)) |
...
- Blob data syncs with an entirely different protocol than documents; one that is asynchronous and built on top of Ditto's rainbow connection technology and optimizes for large binary data transfer. (See [Mesh Networking 101](/v4-8/data-types/#nE9AvJ9QBktFCIcbDS5F-))
...
<Steps>
    <Step>
        Generate the attachment in the Ditto store. (See [Initiating ATTACHMENT Objects](/v4-8/crud/create#SmqTL))
    </Step>
    <Step>
    Reference the attachment token in a document. (See [Referencing Attachment Tokens](/v4-8/crud/create#_TG5N))
    </Step>
</Steps>

To access your attachment data on demand or perform other operations, such as updating or deleting the attachment, first fetch its associated attachment token. (See [Fetching Attachment Tokens](/v4-8/crud/read#34ZMA))
