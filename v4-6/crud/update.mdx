...
When executing `UPDATE` to apply changes, only the minimum data necessary to enforce all peers converge on one view of the data sync across the mesh. ([Updating](/v4-6/crud/update#37pqy))
</Step>

<Step>
`INSERT`

While `INSERT` operations modify all provided fields, even if they remain unchanged. ([Inserting to Update](/v4-6/crud/update#CAGqn))
</Step>
</Steps>
...
<Info>
For the full DQL syntax, see *Ditto Query Language *> [UPDATE](/v4-6/crud/update#8kd9FIMNxcDOqely0ZiHs).
</Info>
...
<Info>
Before you can access an attachment in your app, you must actively fetch it, unlike a document, which is always readily accessible. For instructions, see [READ](/v4-6/crud/read).
</Info>
...
<Info>
For the full DQL syntax, see *Ditto Query Language *> [INSERT](/v4-6/crud/update#oUAljnbafd0Wrz6p2xZCE).
</Info>
