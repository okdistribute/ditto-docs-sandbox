...
<Info>
For a complete overview of the platforms, transports, and devices the JavaScript SDK supports, select **JavaScript Web** from the [Directory of Compatibility Maps](/v4-6/compatibility)﻿.

</Info>
<Info>
If you're using Node.js and want instructions on how to create a task app, see the [Prebuilt Task App](/v4-6/#2zFhnXmxPoCrO4HTyGN2E)﻿ and the [Basic Task App](/v4-6/#tNFB5QLlMjktMDuHnztOm)﻿ tutorial.
</Info>
To install the JavaScript SDK:

<Steps>
  <Step>
    ﻿[Prerequisites](/v4-6/javascript-installing#sb-Fz)﻿: Confirm that you meet the minimum requirements.
  </Step>
  <Step>
  ﻿[Install Dependencies](/v4-6/javascript-installing#i_i6q)﻿: Add the `ditto` package to your environment.
  </Step>
  <Step>
﻿[Integrate Ditto](/v4-6/javascript-installing#Ajrfc)﻿: Import and initialize Ditto in your app.
  </Step>
  <Step>
﻿[Run Ditto](/v4-6/javascript-installing#s71_W)﻿: Authenticate with the Big Peer and start syncing offline.
  </Step>
...
<Info>
  For instructions on how to obtain your access credentials, see [Getting Playground Token Credentials(/v4-6/get-started/sync-credentials)﻿ . For an introduction to authentication in Ditto, see _Security_ > [Cloud Authentication](/v4-6/security/authentication)﻿.
</Info>
...
Add a _sync subscription_ to receive updates on documents that match its query from connected peers in the [mesh](/v4-6/basic/mesh-networking-101)﻿.
...
<Info>
  For a step-by-step demonstration of how to create a basic task app, see the [Basic Task App](/v4-6/javascript-installing) tutorial.
</Info>
```JS JS
import { init, Ditto } from '@dittolive/ditto'
