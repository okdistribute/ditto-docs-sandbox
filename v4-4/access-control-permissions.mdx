...
For instance, senior staff members may have permissions to both read and write documents in the `menuItems` collection, whereas junior staff members are limited to read-only access.&#x20;

You can issue each certificate manually or with the Online with Authentication identity.
...
Currently, you can only specify a permission query on the  field of a document. Permissions on mutable properties are currently not supported.
