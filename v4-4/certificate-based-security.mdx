...
<Steps>
    <Step>
        **Complete a Peer Key Challenge**
            ...
            The peer’s public key is included in the certificates returned by the identity service. The identity service requires proof that the authenticating Small Peer holds the corresponding private key.
            1. Small Peer downloads a challenge token from `/_ditto/auth/cert` (a time-limited JWT treated as opaque data).
            2. Small Peer signs the token using their Peer Key.

    </Step>
    <Step>
        **Log in with Credentials**

            When a client attempts to authenticate, it sends an HTTPS request to the identity service containing:
            - Signed challenge
            - App ID
            - Provider name
            - Credentials to be forwarded to the app's authentication webhook
            The identity service uses the webhook response to build and sign a JWT, which includes an encoded representation of the Ditto Identity based on the webhook response.

    </Step>
    <Step>
        **Upgrade to an X.509 Certificate**

            With the JWT, the peer makes an HTTPS certificate request to the identity service.
            - The request requires a valid JWT.
            - The certificate’s expiration matches the JWT's, and the Ditto identity embedded in the certificate is pulled from the JWT.
            The identity service generates a private key for the device, uses it to create a CSR, and signs it with its CA keys.

    </Step>
    <Step>
            **Certificate Response**
            The identity service responds with:
            - The client's generated certificate and private key
            - A list of CA certificates the client should trust
            - A DateTime for expiration
            The client stores the JWT, private key, device certificate, and CA certificates on disk, updating them as expiration nears. Peers authenticate using either the JWT or device certificate via MTLS, depending on connection type and client capabilities.
