...
<Info>
Later in the code, you'll invoke the `DittoAttachmentToken` to retrieve the `attachment` data from its current storage location, which could be either the Ditto store or the peer-to-peer mesh network. For more information, see [Replicating the Attachment](<./../../../Platform Manual/Data Types.md>), as follows.
</Info>
