...
<Info>
For more information about Kafka, see [Change Data Capture](https://docs.ditto.live/v4-5/js/tutorials/#Zqee8XarjTm0GJPSJbHwY)﻿.
</Info>

## Prerequisites- Basic understanding of Node.js.
- Local installation of [Node 16](https://nodejs.org/en "Node 16").
- A Ditto application syncing with the Big Peer on the [Ditto portal](https://portal.ditto.live/ "Ditto portal") that is on a dedicated cluster.

## Code Sample
See the [github repository](https://github.com/getditto/external-sync/tree/main/nodejs-mongo "github repository") for example code to connect a Node.js instance to the Ditto Big peer as a Kafka sink to MongoDB.
...
```Node.js
async function parseTransaction (database: Db, transaction: DittoTransaction) { const collectionName = transaction.collection const collection = database.collection(collectionName); switch (transaction.type) { case 'requeryRequired': onRequeryRequired() return; case 'documentChanged': onDocumentChanged() default: break; } }
```
