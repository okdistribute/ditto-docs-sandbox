...
description: "Views in SwiftUI can be redrawn frequently. Therefore, it’s important to understand why and when to use the `@StateObject` and the `@ObservedObject` property wrappers when observing an observed object."
---

Both property wrappers tell a SwiftUI View to redraw when the object they are observing updates. The difference between them can be seen when a View is redrawn.
