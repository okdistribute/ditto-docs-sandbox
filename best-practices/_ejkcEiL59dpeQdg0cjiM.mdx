...
When using the Big Peer optional cloud deployment and managing large document collections, you can group peers with similar data access patterns by assigning specific labels, known as _routing hints._
...
<Steps>
  <Step title="Instantiate a `TransportConfig` object:">
<Tabs>
  <Tab title="Swift">
```swift
...
```
  ...
  <Step title="Set the `routingHint` on the `config`object:">
<Tabs>
  <Tab title="Swift">
```swift
var config = DittoTransportConfig()
...
```
  ...
  <Step title="Using the `TransportConfig` object, specify the configuration settings for your Ditto instance:">
<Tabs>
  <Tab title="Swift">
```swift
var config = DittoTransportConfig()
