...
<Info>
Ditto does *not* support nesting documents within documents. Instead, opt for a foreign-key relationship by referencing the document ID. For more information, see [Relationships](/v4-7/document-model/relationships).
</Info>
...
Create the attachment in the Ditto store. ([Initiating ATTACHMENT Objects](/v4-7//crud/create#SmqTL))

</Step>
<Step>

Reference the returned attachment token in a document. ([Referencing Attachment Tokens)](/v4-7//crud/create#_TG5N)
</Step>
</Steps>
...
- In an UPDATE operation — If the MAP does not exist, create it. (See [UPDATE](/v4-7/crud/update))
...
```DQl
1UPDATE COLLECTION cars (properties MAP) 2SET 3 properties \-\> ( 4 color \= 'red' 5 ) 6WHERE 7 \_id \= '123'
